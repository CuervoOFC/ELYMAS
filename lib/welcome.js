...
ʚĭɞ ೃ funcion :: procesamiento de bienvenida y despedida con conexion a base de datos
...
export async function processWelcome(conn, { id, participants, action }) {
    try {
        ...
        for (const jid of participants) {
            ...
            try {
                userAvatar = await conn.profilePictureUrl(jid, 'image')
            } catch {
            ...
            if (action === 'add') {
                ...
                try {
                    const stellarUrl = `https://api.stellarwa.xyz/generate/welcome-image?username=${encodeURIComponent(username)}&guildName=${encodeURIComponent(groupName)}&guildIcon=${encodeURIComponent(groupIcon)}&memberCount=${memberCount}&avatar=${encodeURIComponent(userAvatar)}&background=${encodeURIComponent(groupIcon)}&key=${STELLAR_KEY}`
                    ...
                    console.log('⚠️ StellarWA Welcome falló. Cambiando a EvoGB...', err.message)
                    // 2. Respaldo en EvoGB
                    imageUrl = `${EVO_WELCOME_API}?username=${encodeURIComponent(username)}&memberCount=${memberCount}&guildName=${encodeURIComponent(groupName)}&guildIcon=${encodeURIComponent(groupIcon)}&avatar=${encodeURIComponent(userAvatar)}&background=${encodeURIComponent(groupIcon)}&key=${EVO_KEY}`
                }
            ...
            if (action === 'remove') {
                ...
                // 1. Intentar con StellarWA
                try {
                    const stellarUrl = `${STELLAR_BYE_API}?username=${encodeURIComponent(username)}&guildName=${encodeURIComponent(groupName)}&guildIcon=${encodeURIComponent(groupIcon)}&memberCount=${memberCount}&avatar=${encodeURIComponent(userAvatar)}&background=${encodeURIComponent(groupIcon)}&key=${STELLAR_KEY}`
                    ...
                    console.log('⚠️ StellarWA Bye falló. Cambiando a EvoGB...', err.message)
                    // 2. Respaldo en EvoGB
                    imageUrl = `${EVO_BYE_API}?username=${encodeURIComponent(username)}&memberCount=${memberCount}&guildName=${encodeURIComponent(groupName)}&guildIcon=${encodeURIComponent(groupIcon)}&avatar=${encodeURIComponent(userAvatar)}&background=${encodeURIComponent(groupIcon)}&key=${EVO_KEY}`
                }
