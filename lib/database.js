...
ʚĭɞ 💤 funcion :: manejador de JSONs con cache en memoria (RAM)
──────✧✦✧──────
*/
...
function getGroup(jid) {
    ...
    if (!groups[jid]) {
        groups[jid] = {
            ...
            antilink: false,
            nsfw: false,
            antinsfw: false, // 👈 Se añade la clave para AntiNSFW aquí
            economy: true,
            createdAt: Date.now()
        }
        saveGroups(groups)
    } else if (groups[jid].antinsfw === undefined) {
        // Asigna valor por defecto si el grupo ya existía previamente en el JSON
