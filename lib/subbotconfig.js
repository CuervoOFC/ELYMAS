...
export function getSubbotConfig(botJid, defaultConfig) {
    ...
    try {
        ...
        if (cleanNumber && data[cleanNumber]) {
            return {
                name: data[cleanNumber].name || defaultConfig?.botName || config?.botName || 'Cuervo',
                ownerName: data[cleanNumber].ownerName || defaultConfig?.ownerName || 'TheDevil',
                image: data[cleanNumber].image || null
            }
        }
