...
ʚĭɞ 💤 funcion :: AntiNSFW con conversor EvoGB (WebP -> PNG / GIF -> MP4)
──────✧✦✧──────
*/
...
// Convertir imágenes/stickers/gifs a PNG/MP4 vía EvoGB API
async function convertMedia(mediaUrl, targetFormat = 'png') {
    try {
...
async function processAntiNSFW(m, conn, isOwner) {
    ...
    try {
        ...
        // 2. Subida del archivo original a los servidores
        let mediaUrl = null
        ...
        // 3. Conversión de formato si es Sticker (webp) o Video/GIF
        ...
        // 4. Consulta de análisis a la API AntiNSFW de EvoGB
        ...
        // 5. Acción en caso de detección NSFW
        if (json.analysis.is_nsfw) {
            ...
            // A. Eliminar mensaje con el contenido explícito
            try {
            ...
            // B. Enviar advertencia de expulsión
            const flag = json.analysis.flag || 'NSFW'
            const confidence = json.analysis.confidence || '100%'

            await conn.sendMessage(m.chat, {
                text: `🔞 *@${cleanNumber}*, el contenido enviado fue detectado como explícito (*${flag}* - ${confidence}) y has sido eliminado del grupo.`,
                mentions: [realJid]
            }).catch(() => {})

            // C. Expulsar al usuario
            try {
