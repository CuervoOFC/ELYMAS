...
ʚĭɞ ೃ codigo :: plugins/grupos/demote.js
ʚĭɞ ೃ funcion :: degrada de administrador a un usuario
──────✧✦✧──────
*/
...
export default {
    ...
    async run(m, { conn, args }) {
        ...
        try {
            await conn.groupParticipantsUpdate(m.chat, [target.mentionId], 'demote')
            ...
            const responseText = 
                ...
                `│ 📌 *Estado:* Ya no es Administrador del grupo.\n` +
                `│\n` +
                `╰──────────────`
            ...
            console.error('❌ Error en demote.js:', error)
            return m.reply('❌ No se pudo degradar al usuario.')
        }
    }
}
